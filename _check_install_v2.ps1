...
$expectedMd5 = 'e934e92ba20474c39cab4c07f6208d18'  # v0.19.0.43 binary (UI 真修: IME ImmReleaseContext + drag screen coords + outer AdjustWindowRect)
$expectedInstallerMd5 = '85b985346399c40b691f4c5e682f17a9'  # v0.19.0.43 installer
$expectedBuildTime = '2026-07-21 12:07:00'  # v0.19.0.43 build (NSIS timestamp)
...
foreach ($p in $paths) {
    if (Test-Path $p) {
        ...
        $match = if ($h -eq $expectedMd5) { '<== MATCHES expected (v0.19.0.43 UI 真修)' } else { '<== MISMATCH (old binary or wrong build!)' }
        Write-Host "  $p"
...
Write-Host "  Expected: md5=$expectedMd5 (v0.19.0.43 UI 真修, build 2026-07-21)"
Write-Host ""
...
Write-Host "  Installer v0.19.0.43 md5 expected: $expectedInstallerMd5"
...
Write-Host "    [5] newest file 是 7/21 12:07 → installer 装过; 是更早 → 没覆盖"
