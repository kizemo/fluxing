...
$expectedMd5 = '5dba1bcd00320da7efe988abc8ddec4b'  # v0.19.0.47 binary (Phase I Bug 4 续修: OnCreate SetFocus(s_hInput) 触发 TSF attach)
$expectedInstallerMd5 = 'd201e6511e63eeac45253f909f8540ae'  # v0.19.0.47 installer
$expectedBuildTime = '2026-07-22 23:08:00'  # v0.19.0.47 build (NSIS timestamp)
...
foreach ($p in $paths) {
    if (Test-Path $p) {
        ...
        $match = if ($h -eq $expectedMd5) { '<== MATCHES expected (v0.19.0.47 Phase I Bug 4 续修)' } else { '<== MISMATCH (old binary or wrong build!)' }
        Write-Host "  $p"
...
Write-Host "  Expected: md5=$expectedMd5 (v0.19.0.47 Phase I Bug 4 续修, build 2026-07-22)"
Write-Host ""
...
Write-Host "  Installer v0.19.0.47 md5 expected: $expectedInstallerMd5"
