...
$expectedMd5 = 'bcdee00435ac3b27e8c81a20494264f7'  # v0.19.0.42 binary (Stop hook 修补: OnGetMinMaxInfo DPI 缩放 + drag e2e)
$expectedInstallerMd5 = '656e6894e8d15e3065d39519c5fa1ba1'  # v0.19.0.42 installer
$expectedBuildTime = '2026-07-21 11:13:00'  # v0.19.0.42 build (NSIS timestamp)
...
foreach ($p in $paths) {
    if (Test-Path $p) {
        ...
        $match = if ($h -eq $expectedMd5) { '<== MATCHES expected (v0.19.0.42 Stop hook 修补)' } else { '<== MISMATCH (old binary or wrong build!)' }
        Write-Host "  $p"
...
Write-Host "  Expected: md5=$expectedMd5 (v0.19.0.42 Stop hook 修补, build 2026-07-21)"
Write-Host ""
...
Write-Host "  Installer v0.19.0.42 md5 expected: $expectedInstallerMd5"
...
Write-Host "    [5] newest file 是 7/21 11:13 → installer 装过; 是更早 → 没覆盖"
