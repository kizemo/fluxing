...
### 1.7 v2 范围限制（与 spec 004 §1.1 对齐）

- **不含输入历史**：spec 004 明确"v2 同步范围不含输入历史"（隐私 + 数量巨大 + 无业务价值）。后续 v3+ 评估。
- **不含临时状态**：当前输入、上屏缓冲、session 状态等只在本地。
- **不含 OS 相关设置**：DPI、显示器、locale 等系统级设置不同步。
- **大小估算（单端）**：
  - `user.db`（RIME 文本格式）：典型 100KB - 1MB（用户词典 1000-10000 词条）。
  - `phrases.json`：典型 5-50KB（100-500 条短语）。
  - `<schema>.schema.yaml`：典型 2-10KB 每个方案。
  - `default.yaml` + `weasel.yaml`：典型 5-15KB。
  - **单端 sync-package 总大小：约 200KB - 2MB**（压缩后约 50-500KB）。
- **频率限制**：每用户每日 push 100 次（防滥用 + 节省 Vercel 用量）。
