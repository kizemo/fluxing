# 020 - TestUserDictUpdate (integration test for spec 008) (placeholder)

> Scope: TDD.md sec 3.1 integration test #X for spec 008.
> This spec is a PLACEHOLDER. The production code that the test would
> exercise does not exist yet (spec 008 is spec-stage only), so the
> test cannot be implemented until that spec ships.
> This placeholder exists so the v0.18.13.0 release records the
> tracking item.

## 0. Status: BLOCKED on spec 008

- spec 008 production code: NOT YET SHIPPED.
...
When spec 008 ships its production code:
1. Create test/Integration/Testintegration-test-user-dict-update/integration-test-user-dict-update.cpp with the mock pattern
   from spec 018 L25 (mock librime objects + behavior assertions).
2. Create test/Integration/Testintegration-test-user-dict-update/integration-test-user-dict-update.vcxproj following
   TestBindingResolution.vcxproj template.
3. Add Testintegration-test-user-dict-update to scripts/run-tests.bat for loop.
...
- Testintegration-test-user-dict-update.exe reports N/N PASS, exit 0.
