"""Build v0.19.0.25: L83 - DPI scaling + WIC BitmapScaler + mac glass aesthetic."""
...
OVERRIDES = {
    ...
    "WEASEL_BUILD": "25",
    "PRODUCT_VERSION": "0.19.0.25",
    "FILE_VERSION": "0.19.0.25",
    "VERSION_MAJOR": "0",
    "VERSION_MINOR": "19",
    "VERSION_PATCH": "0",
...
def main():
    print(f"[build-v0.19.0.25] ROOT={ROOT}")
    vcvars_env = dump_env_from_vcvars()
    print(f"[build-v0.19.0.25] got {len(vcvars_env)} vars")
    ...
    if small_logo_src.exists():
        for dest_dir in [ROOT / "output" / "Win32", ROOT / "output"]:
            ...
            print(f"[build-v0.19.0.25] copied logo -> {dest_dir / 'fluxing-logo_small.png'}")

    print(f"[build-v0.19.0.25] NSIS install.nsi ...")
    makensis = NSIS_DIR / "Bin" / "makensis.exe"
    res = subprocess.run(
        [str(makensis),
         "/DFLUXING_VERSION=0.19.0",
         "/DWEASEL_BUILD=25",
         "/DPRODUCT_VERSION=0.19.0.25",
         "install.nsi"],
        cwd=str(ROOT / "output"),
        env=merged,
        capture_output=True, text=True, timeout=120,
    )
    print(f"[build-v0.19.0.25] NSIS exit={res.returncode}")
    ...
    src = ROOT / "output" / "archives" / "fluxing-0.19.0.25-installer.exe"
    dst = ROOT / "release" / "fluxing-0.19.0.25-installer.exe"
    if src.exists():
        shutil.copy2(src, dst)
        print(f"[build-v0.19.0.25] copied -> {dst}")
        sz = dst.stat().st_size
        print(f"[build-v0.19.0.25] size: {sz} bytes")
        h = hashlib.sha256()
        with open(dst, "rb") as f:
            for chunk in iter(lambda: f.read(65536), b""):
                h.update(chunk)
        print(f"[build-v0.19.0.25] sha256: {h.hexdigest()}")
    else:
        print(f"[build-v0.19.0.25] WARN: src {src} not found")
    return 0
